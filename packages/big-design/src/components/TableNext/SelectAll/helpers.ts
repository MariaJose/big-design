...
export function selectAll<T>({
  ...
  items,
  selectedItems,
}: SelectAllRowsArg<T>): TableSelectable['selectedItems'] {
  ...
  items.forEach((parentRow, parentRowIndex) => {
    ...
    if (isExpandable) {
      const childrenRows = getChildrenRows(parentRow, expandedRowSelector);

      childrenRows.forEach((_childRow, childRowIndex) => {
        if (newSelectedItems[`${parentRowIndex}.${childRowIndex}`] === undefined) {
